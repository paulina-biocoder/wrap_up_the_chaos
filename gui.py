...
class Gui():
  ...
  def open_books(self):
    if self.toplevel_window is None or not self.toplevel_window.master.winfo_exists():
      ...
      self.toplevel_window.master.mainloop()
  ...
  def open_movies(self):
    if self.movies_window is None or not self.movies_window.child.winfo_exists():
      ...
      self.movies_window.child.mainloop()
...
class CategorizeWindow():
  def __init__(self, master, books, loaded_books):
    ...
    self.books = books
    self.loaded_books = loaded_books
  ...
  def borrowed_book(self):
    ...
    deadline_date = simpledialog.askstring("Borrow Book", "When do you have to return this book? ").capitalize()
    borrowed_book = {
      'title': borrow_book,
      'status': 'Borrowed',
      'person': borrower,
    ...
    for loaded_book in self.loaded_books:
      for _,_ in loaded_book.items():
        if n_book == loaded_book['title'] and loaded_book['status'] == 'Lent':
          ...
          messagebox.showinfo('Info', "Chosen book hasn't status Lent")
          book_found = True
          return self.loaded_books
    if not book_found:
        messagebox.showinfo('Info', 'Book not found')
...
class Movies():
  def __init__(self, root):
    ...
    button5 = ctk.CTkButton(self.child, text='Load form file', font=('Arial', 16), command=self.load_results,
                            fg_color="DeepPink")
    button5.pack(pady=5)

    button6 = ctk.CTkButton(self.child, text="Go back", font=('Arial', 16), command=self.go_back, fg_color="DeepPink")
    button6.pack(pady=5)
  ...
  def show_movies(self):
    if self.movies:
      ...
      loaded_movie_info = "\n".join([" | ".join([f'{k}: {v}' for k, v in loaded_movie.items() if v != '']) for loaded_movie in self.loaded_movies])
      messagebox.showinfo('Movies', loaded_movie_info)
    else:
  ...
  def load_results(self):
    filename = simpledialog.askstring("Add movies", 'Enter path to the file: ')
    try:
      with open(filename, 'r') as file:
  ...
  def open_categorize(self):
    if self.categorize is None or not self.categorize.winfo_exists():
      self.categorize = Categorize(self.child, self.movies, self.loaded_movies)
...
class Categorize(ctk.CTkToplevel):
  def __init__(self, child, movies, loaded_movies):
    super().__init__(child)
    self.title('Categorize Movies')
    self.geometry('260x150')
    self.movies = movies
    self.loaded_movies = loaded_movies
